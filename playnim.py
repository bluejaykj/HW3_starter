...
python playnim <heaps> <player1> <player2> <heuristic>
  ...
  <player1> and <player2> are optional and if not given default to a
    ...
    mm: for a simple minimax algorithm
    ab:  for the alphabeta version
    abN, where N is an integer between 1 and 10, for a player that
...
# that implement them.  A player function takes two arguments, a game and a state.
# We add to the PLAYERS dictionary player function named ab1,ab2,...ab10
# that use alpha_beta_search with depth cutoffs between 1 and 10
...
def play_nim(heaps, player1, player2, heuristic):
    """ Play a NIM game using player1 and player 2 with an initial
    ...
    print(f"NIM with heaps {heaps} using players {player1} and {player2} and heuristic {heuristic}" )
